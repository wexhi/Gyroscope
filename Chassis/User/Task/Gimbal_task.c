...
static void RC_gimbal_control();      // 遥控器控制云台电机
static void gimbal_yaw_control();     // 锁云台模式
static void RC_Yaw_speed();           // 遥控器控制Yaw轴速度
static void RC_Yaw_control();         // 锁云台模式下Yaw轴控制
static void RC_Pitch_control();       // Pitch轴控制
static void detel_calc(fp32 *angle);  // 角度处理
static void detel_calc2(fp32 *angle); // 角度处理
...
{
    if (rc_ctrl.rc.s[0] == 1)
        RC_gimbal_control(); // 速度模式
    else
        gimbal_yaw_control(); // 锁云台模式
}
